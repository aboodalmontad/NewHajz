...
const LoginSelector: React.FC<{ onLogin: (view: string, employee?: Employee) => void }> = ({ onLogin }) => {
    ...
    // مراقبة حالة الربط المحلي - بمجرد النجاح، نحفظ الحالة ونعود للرئيسية
    useEffect(() => {
        if (meshStatus === 'connected') {
            alert("✅ تم الربط المحلي بنجاح! سيتذكر الجهاز هذا الارتباط دائماً.");
            setLoginMode('none');
    ...
    const handleCloudJoin = async () => {
        ...
        if (success) {
            alert("✅ تم الربط السحابي الدائم بنجاح! سيعمل الجهاز تلقائياً عند فتحه المرة القادمة.");
            setLoginMode('none');
        } else {
    ...
    // التحقق مما إذا كان الجهاز مرتبطاً سابقاً
        ...
        <div className="max-w-6xl mx-auto text-center mt-8 pb-20 px-4 animate-in fade-in duration-500">
            ...
            {(isLinked || meshStatus === 'connected') && (
                <div className="flex flex-col items-center justify-center gap-2 mb-12">
                    <div className="flex items-center gap-2 bg-green-500/10 border border-green-500/30 px-6 py-3 rounded-2xl">
                        <span className="flex h-3 w-3 rounded-full bg-green-500 animate-pulse"></span>
                        <span className="text-green-400 text-sm font-bold">هذا الجهاز مرتبط بنظام نشط</span>
                        {state?.syncId && (
                            <span className="bg-slate-800 text-sky-400 px-3 py-1 rounded-lg text-xs font-mono font-bold border border-slate-700 ml-2">{state.syncId}</span>
                        )}
                    </div>
                </div>
            )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {!(isLinked || meshStatus === 'connected') ? (
                        <Card className="bg-slate-800 p-8 cursor-pointer border-2 border-dashed border-sky-500/30 hover:bg-sky-500/5 group" onClick={() => setLoginMode('sync_join')}>
                            ...
                            <p className="text-slate-500 text-sm mt-2">لمزامنة هذا الجهاز مع نظام قائم (لمرة واحدة)</p>
                        </Card>
                    ) : (
                        <Card className="bg-slate-900 p-8 border border-slate-700 flex flex-col justify-center">
                            <h3 className="text-xl font-bold text-slate-400 mb-4">إدارة الربط النشط</h3>
                            <Button variant="danger" size="sm" className="opacity-60 hover:opacity-100" onClick={() => {
                                if(confirm("هل أنت متأكد من قطع الارتباط؟ سيتوقف الجهاز عن استلام التحديثات.")) disconnectSync();
